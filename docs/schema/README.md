...
| `ui:idFragment` | nein | URI-Fragment für einen `@id`-Anker (siehe [../rendering.md](../rendering.md#id-mechanismus)) | `AccountingService`, `LegalService`, `LocalBusiness`, `MedicalBusiness`, `ProfessionalService`, `NGO`, `Organization` (alle: `"organization"`), `Person` (`"person"`) |
...
zum Einsatz — beide bestehenden `id_reference_or_literal`-Felder
(`Event.organizer`, `JobPosting.hiringOrganization`) verwenden für ihr
einziges Literal-Feld (`name`) den Default-Label-Schlüssel `label_name`.
...
| `id_reference_or_literal` | `ui:literalFields`, `ui:literalFieldPlaceholders`, `ui:literalType` | Objekt `{_mode: "reference", _fragment: "…"}` oder `{_mode: "literal", <literalFields>…}` |
